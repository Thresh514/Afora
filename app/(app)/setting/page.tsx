...
function SettingPage() {
  ...
  const [profileEditField, setProfileEditField] = useState<ProfileEditField | null>(null);
  ...
  const { ref: rhfDisplayNameRef, ...displayNameInputProps } = register("displayName", {
  ...
  const cancelProfileInlineEdit = useCallback(() => {
    ...
    if (profileEditField === "name") {
      ...
      setProfileValue("demographic", (user.unsafeMetadata?.demographic as string) || "");
    }
        ...
        >
          <div className="flex flex-col items-center gap-5">
            ...
            <div className="text-center">
              <p className="settings-display text-lg font-semibold text-foreground">Loading</p>
      ...
      <header className="relative overflow-hidden border-b border-border bg-muted/40">
        ...
        <div className="relative mx-auto max-w-6xl px-4 py-10 md:px-8 md:py-14">
          <div className="grid items-end gap-10 lg:grid-cols-[minmax(0,1fr)_320px] lg:gap-6">
            <div className="settings-enter max-w-xl space-y-4" style={{ animationDelay: "0.08s" }}>
              ...
              <div className="flex flex-wrap items-end gap-4">
                ...
                <span className="hidden h-px flex-1 min-w-[3rem] bg-border sm:block" aria-hidden />
              </div>
              ...
              >
                <CardHeader className="space-y-2 border-b border-border px-6 py-6 md:px-8">
                  <CardTitle className="flex items-center gap-2 text-xl font-semibold tracking-tight text-foreground">
                    <User className="h-5 w-5 shrink-0 text-[var(--afora)]" aria-hidden />
                    Personal profile
                  </CardTitle>
                ...
                <CardContent className="p-0">
                  <section className="px-6 py-6 md:px-8" aria-labelledby="personal-profile-heading">
                    <form onSubmit={handleProfileSubmit(onProfileSubmit)} className="space-y-5">
                      <h3 id="personal-profile-heading" className="sr-only">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="space-y-1.5">
                          <Label
                            htmlFor={
                              profileEditField === "name" ? "settings-display-name" : undefined
                            }
                          ...
                          {profileEditField === "name" ? (
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                              <p className="min-h-9 max-w-[calc(100%-2.5rem)] truncate py-2 text-base font-medium leading-snug text-foreground">
                                {(displayNameWatch && displayNameWatch.trim()) || "Not set"}
                              </p>
                              <Button
                                type="button"
                                variant="ghost"
                          ...
                          <p className="text-xs text-muted-foreground">Shown across Afora.</p>
                        </div>
                        ...
                        {DEMO_SELECT_FIELDS.map(({ key, label, options, inputId }) => {
                          const isOpen = profileEditField === key;
                          const value = watch(key);
                          const fieldError = profileErrors[key];
                          return (
                            <div key={key} className="space-y-1.5">
                              <Label htmlFor={isOpen ? inputId : undefined} className={fieldLabel}>
                                {label}
                              </Label>
                              {isOpen ? (
                                <div className="flex flex-wrap items-center gap-2">
                        ...
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-6">
                          <div className="min-w-0">
                            ...
                            <p className="mt-0.5 text-sm text-muted-foreground">
                              {animationsEnabled
                                ? "Confetti and task-complete animation on tasks, claims, and surveys."
                  ...
                  <CardContent className="pt-6">
                    <div className="divide-y divide-border rounded-lg border border-border">
                      <div className="flex flex-col gap-2 p-4 sm:flex-row sm:items-center sm:justify-between sm:gap-6">
                        ...
                        <Switch
                          id="admin-all-switch"
                          checked={allowMatching && allowTaskAssignment && allowGroupEvaluation}
                          onCheckedChange={(checked) => void handleAllAdminParticipation(checked)}
