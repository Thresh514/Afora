...
function StagePage() {
  ...
  // 恢复原有逻辑：存储从后端获取的过期任务
  const [backendOverdueTasks, setBackendOverdueTasks] = useState<
    Array<{
      id: string;
      title: string;
      description: string;
  ...
  const handleNewTask = async () => {
    ...
    const softDeadline =
      (document.getElementById("soft-deadline") as HTMLInputElement)?.value || "";
    const hardDeadline =
      (document.getElementById("hard-deadline") as HTMLInputElement)?.value || "";
    const points = parseInt(
      (document.getElementById("task-points") as HTMLInputElement)?.value || "1"
    );
  ...
  const handleSwapTaskConfirm = async () => {
    ...
    startTransition(async () => {
      const result = await reassignTask(projId, stageId, currentTaskId, swapAssigneeEmail.trim());
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
              ...
              <div className="flex-1 space-y-4">
                ...
                <div className="flex items-center justify-between mb-2">
                  <h2 className="text-xl md:text-2xl font-semibold text-white">Tasks List</h2>
                </div>
              </div>
            </div>
          </div>
        ...
        <AlertDialog open={isCreateTaskOpen} onOpenChange={setIsCreateTaskOpen}>
          ...
          <AlertDialogContent className="w-full max-w-md">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>Enter the details for the new task.</AlertDialogDescription>
            </AlertDialogHeader>
            <div className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input id="soft-deadline" type="date" className="w-full" />
                </div>
                <div>
                  <Label htmlFor="hard-deadline" className="text-sm font-medium text-foreground">
                    Hard Deadline
                  </Label>
                  <Input id="hard-deadline" type="date" className="w-full" />
                </div>
              </div>
              <div>
