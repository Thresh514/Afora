          ...
          items: {
            ...
            properties: {
              ...
              tasks: {
                type: "array",
                description:
                  "A list of tasks associated with this stage. The number of tasks in each stage should match the number of team members.",
                minItems: 1,
                items: {
                  type: "object",
                  properties: {
                    ...
                    assigned_member: {
                      type: "string",
                      description:
                        "Email address of the team member assigned to this task based on their skills and interests",
                      minLength: 1,
                    },
                    assignment_reason: {
                      type: "string",
                      description:
                        "Brief explanation of why this member was assigned to this task based on their capabilities and interests",
                      minLength: 1,
                    },
                  },
                  required: [
...
const sanitizeAndParseJSON = (jsonString) => {
  ...
  try {
    ...
    parsed.stages = parsed.stages.map((stage, index) => {
      ...
      stage.tasks = stage.tasks.map((task, taskIndex) => {
        if (!task.task_name || typeof task.task_name !== "string") {
          throw new Error(
            `Task ${taskIndex + 1} in stage "${stage.stage_name}" has invalid or missing name`
          );
        }

        if (!task.task_description || typeof task.task_description !== "string") {
          throw new Error(
            `Task ${taskIndex + 1} (${task.task_name}) in stage "${stage.stage_name}" has invalid or missing description`
          );
        }
        ...
        if (!dateRegex.test(task.soft_deadline)) {
          throw new Error(
            `Task ${taskIndex + 1} (${task.task_name}) has invalid soft deadline format`
          );
        }
        if (!dateRegex.test(task.hard_deadline)) {
          throw new Error(
            `Task ${taskIndex + 1} (${task.task_name}) has invalid hard deadline format`
          );
        }
        ...
        if (!task.assigned_member || typeof task.assigned_member !== "string") {
          throw new Error(
            `Task ${taskIndex + 1} (${task.task_name}) has invalid or missing assigned member`
          );
        }
        if (!task.assignment_reason || typeof task.assignment_reason !== "string") {
          throw new Error(
            `Task ${taskIndex + 1} (${task.task_name}) has invalid or missing assignment reason`
          );
        }
    ...
    console.error(
      ...
      jsonString?.substring(0, 1000) || "No response"
    );
    throw error;
  }
};
...
export const generateTask = async (
  ...
  try {
    ...
    if (
      !projectInfo.projectPurpose ||
      !projectInfo.keyMilestones ||
      !timelineInfo.projectDuration
    ) {
      throw new Error(
        "Missing required project information. Please complete questions 1, 2, and 3 in the team charter."
      );
    }
    ...
    if (sanitizedData.stages) {
      sanitizedData.stages.forEach((stage, stageIndex) => {
        ...
        if (stage.tasks.length !== teamSize) {
          ...
          throw new Error(
            `Stage ${stageIndex + 1} (${stage.stage_name}) has ${stage.tasks.length} tasks but should have exactly ${teamSize} tasks (one per team member)`
          );
        }
        ...
        stage.tasks.forEach((task, taskIndex) => {
          ...
          if (assignedMembers.has(task.assigned_member)) {
            ...
            throw new Error(
              `Stage ${stageIndex + 1} (${stage.stage_name}): Member ${task.assigned_member} is assigned to multiple tasks. Each member should be assigned exactly one task per stage.`
            );
          }
          ...
          if (!memberExists) {
            ...
            throw new Error(
              `Stage ${stageIndex + 1} (${stage.stage_name}), Task ${taskIndex + 1}: Assigned member ${task.assigned_member} is not in the team member list`
            );
          }
        });
