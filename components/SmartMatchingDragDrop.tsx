...
function DraggableMemberItem({ id, email, isAdmin }: DraggableMemberItemProps) {
  ...
  if (isAdmin) {
    // 管理员不可拖拽，只显示
    return (
      <div className="flex items-center gap-2 p-2 rounded-md transition-all duration-200 bg-red-100 border border-red-300 cursor-default shadow-sm">
...
function ProjectContainer({
  ...
  const totalCount =
    effectiveAdmins.length + effectiveMembers.length + remainingProposedMembers.length;
  ...
  return (
    ...
    >
      ...
      <div className="mb-3 pointer-events-none">
        ...
        <div className="text-sm text-gray-500">{totalCount} members</div>
        {/* 显示匹配原因 */}
        {matchingReasoning && (
          <div className="mt-1 text-xs text-gray-600 italic">{matchingReasoning}</div>
        )}
      </div>
...
export default function SmartMatchingDragDrop({
  ...
  const parseDragId = (
    id: string
  ): { projectId: string; email: string; type: "admin" | "member" | "proposed" } | null => {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (targetProject) {
      ...
      if (
        effectiveMembers.includes(activeParsed.email) ||
        effectiveAdmins.includes(activeParsed.email)
      ) {
        showErrorToast({
          type: "smart_matching",
    ...
    if (activeParsed.type === "proposed") {
      ...
      const isManuallyAdded = Object.values(previewChanges).some(
        (changes) =>
  ...
  const getProjectEffectiveData = (project: (typeof projects)[0]) => {
    ...
    const effectiveMembers = Array.from(new Set(effectiveMembersRaw)).filter(
      (m) => !effectiveAdmins.includes(m)
    );
    ...
    const remainingProposedMembers = Array.from(new Set(proposedNewMembers))
      .filter(
        (email: string) =>
          !Object.values(previewChanges).some(
            (changes) => changes.addedMembers.includes(email) || changes.addedAdmins.includes(email)
          )
      )
      .filter(
        (email: string) => !effectiveMembers.includes(email) && !effectiveAdmins.includes(email)
      );
