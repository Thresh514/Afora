      ...
      (async () => {
        try {
          ...
          if (!membersData.success || !membersData.data || membersData.data.length === 0) {
            toast.error(
              "No team member survey responses found. All team members must complete both app onboarding and project onboarding surveys before analysis can proceed."
            );
            return;
          }
          ...
          for (const member of membersData.data) {
            ...
            const hasCompleteApp =
              appResponses.length >= requiredAppQuestions &&
              appResponses.every((resp: string) => resp && resp.trim() !== "");
            const hasCompleteProj =
              projResponses.length >= requiredProjQuestions &&
          ...
          if (incompleteMembers.length > 0) {
            const missingDetails = membersWithMissingData
              .map((m) => {
                const missing = [];
          ...
          const memberResponses = membersData.data.map((member: MemberData) => {
            // onboardingResponses
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {projectFilter ? "Team Compatibility Analysis" : "Team Compatibility Analysis"}
            </CardTitle>
            <CardDescription>
              Analyze team&apos;s overall compatibility and collaboration potential based on member
              onboarding surveys.
              <div className="mt-2 text-sm text-amber-600 font-medium">
                ⚠️ All team members must complete both app onboarding and project onboarding surveys
              ...
              <div className="space-y-6">
                  ...
                  <div className="mx-auto max-w-5xl rounded-xl border border-border bg-gradient-to-br from-muted/70 to-card p-6">
                    <div className="text-center mb-6">
                      ...
                      {(() => {
                        ...
                        const overallScore = curveScore(
                          Number.isFinite(baseOverall) ? baseOverall : 0
                        );
                        return (
                          <>
                            <div
                              className={`text-5xl font-bold ${getScoreColor(overallScore)} mb-2`}
                            >
                              {overallScore}
                            </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                      ...
                      <div className="text-center">
                        <div className="text-2xl font-bold text-green-600">
                          {curveScore(
                            toSafeNumber(analysis.score_breakdown.communication_alignment)
                          )}
                        </div>
                      ...
                      <div className="text-center">
                        <div className="text-2xl font-bold text-orange-600">
                          {curveScore(
                            toSafeNumber(analysis.score_breakdown.work_style_compatibility)
                          )}
                        </div>
                      ...
                      <div className="text-center">
                        <div className="text-2xl font-bold text-indigo-600">
                          {curveScore(
                            toSafeNumber(analysis.score_breakdown.schedule_compatibility)
                          )}
                        </div>
                    ...
                    <div className="rounded-lg border border-blue-500/25 bg-gradient-to-br from-blue-500/10 to-card p-4">
                      ...
                      <div className="rounded-md border border-blue-500/20 bg-blue-500/10 px-3 py-2 text-sm leading-relaxed text-blue-900 dark:text-blue-100">
                        {analysis.team_analysis.collaboration_potential ||
                          "Collaboration potential not assessed"}
                      </div>
                    </div>
                  </div>
                )}
                ...
                {analysis.team_analysis && (
                  <div className="mx-auto max-w-5xl rounded-lg border border-purple-500/25 bg-gradient-to-br from-purple-500/10 to-card p-4">
                    <h4 className="mb-3 font-semibold text-purple-800 dark:text-purple-300">
                      Key Recommendations
                    </h4>
                    <div className="space-y-2">
                        ...
                        >
                          <div className="mb-3 flex items-center justify-between">
                            ...
                            <div className="text-right">
                              {(() => {
                                const memberScore = curveScore(
                                  toSafeNumber(member.compatibility_score)
                                );
                                return (
                                  <div
                                    className={`text-lg font-bold ${getScoreColor(memberScore)}`}
                                  >
                                    {memberScore}
                                  </div>
                                );
