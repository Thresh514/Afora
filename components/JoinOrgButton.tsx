...
function JoinOrgButton({
    ...
    startTransition(async () => {
      const { success, message } =
        user && user.id && user.primaryEmailAddress
          ? await inviteUserToOrg(orgCode, user.primaryEmailAddress.emailAddress, "member")
          : { success: false, message: "user does not exist" };
      if (success && orgCode && orgCode.trim()) {
        // console.log("Successfully joined");
        setIsOpen(false);
        router.push(`/org/${orgCode}`);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Join a Group</DialogTitle> {/* Updated title */}
        ...
        <DialogFooter>
          <Button type="submit" onClick={handleJoinNewOrganization} disabled={isPending}>
            {" "}
            {/* Updated onClick */}
