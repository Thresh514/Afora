...
const ProjectPage = ({ id, projId }: { id: string; projId: string }) => {
  ...
  const [responses, setResponses] = useState<string[]>(
    new Array(teamCharterQuestions.length).fill("")
  );
  ...
  useEffect(() => {
    if (teamCharterData && !loading && !error) {
      ...
      if (savedResponses.length > 0) {
        ...
        const cleanResponses = new Array(teamCharterQuestions.length)
          .fill("")
          .map((_, index) => savedResponses[index] || "");
  ...
  useEffect(() => {
    ...
    stages.forEach((stage, i) => {
      newStageStatus[i] =
        i > 0 && newStageStatus[i - 1] !== 2 ? 0 : stage.tasksCompleted == stage.totalTasks ? 2 : 1;
    });
  ...
  const projectMembers = useMemo(() => {
    ...
    const allMembers = Array.from(
      new Set([...(proj.members || []), ...(proj.admins || []), ...(proj.adminsAsUsers || [])])
    );
  ...
  const handleTeamCharterSave = () => {
    ...
    startTransition(async () => {
      try {
        ...
        const missingRequiredResponses = requiredQuestionIndices.filter(
          (index) => !responses[index] || responses[index].trim().length === 0
        );
        ...
        if (errorMessage.includes("Unauthorized")) {
          errorMessage =
            "You don't have permission to save this team charter. Please contact an admin.";
  ...
  const handleEditSave = () => {
    try {
      ...
      const stagesToDelete = stages.filter(
        (stage) => !reorderedStages.some((reorderedStage) => reorderedStage.id === stage.id)
      );
  ...
  const analysisParticipants = Array.from(
    new Set([...(proj?.members || []), ...(proj?.adminsAsUsers || [])])
  );
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
              ...
              <div className="flex-1 space-y-4">
                ...
                <div className="flex items-center justify-between mb-2">
                  <h2 className="text-xl md:text-2xl font-semibold text-white">Team Overview</h2>
                    ...
                    <div className="bg-white bg-opacity-20 backdrop-blur-sm rounded-lg px-4 py-3 inline-flex items-center gap-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-bold text-white text-lg min-w-[3rem]">
                          {Math.round(
                            (stages.reduce((acc, stage) => acc + stage.tasksCompleted, 0) /
                              stages.reduce((acc, stage) => acc + stage.totalTasks, 0)) *
                              100
                          )}
                          %
                        </span>
              ...
              {stages.length === 0 ? (
                <div className="bg-card rounded-lg shadow-sm border p-8 text-center space-y-6">
                  ...
                  <div className="flex justify-center gap-4">
                    ...
                    />
                    <AlertDialog open={isTeamCharterOpen} onOpenChange={setIsTeamCharterOpen}>
                      <AlertDialogTrigger asChild>
                        <Button onClick={() => setIsProUpgradeOpen(true)} variant="outline">
                      ...
                      <AlertDialogContent className="w-full max-w-4xl">
                        ...
                        <AlertDialogFooter>
                          <Button onClick={() => setIsTeamCharterOpen(false)} variant="outline">
                            Cancel
                          </Button>
                  ...
                  >
                    <AlertDialogContent className="w-full max-w-2xl">
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-sm text-red-700">
                                {charterSaveError?.message ||
                                  "Please complete all required questions before saving."}
                              </p>
                            </div>
                          </div>
                        </div>
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                              ...
                              <ul className="text-sm text-blue-700 space-y-1">
                                <li>
                                  • Question 1: What is the main mission/vision of the project?
                                </li>
                                <li>• Question 2: List key project milestones</li>
                                <li>• Question 3: Expected project duration (in weeks)</li>
                              </ul>
                              <p className="text-sm text-blue-600 mt-2">Questions 4 is optional.</p>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <AlertDialogFooter>
                        <Button onClick={() => setShowCharterErrorDialog(false)} variant="outline">
                          Close
                        </Button>
                      </AlertDialogFooter>
                    </AlertDialogContent>
                ...
                <div className="space-y-6">
                  {isEditing ? (
                    <div className="bg-card rounded-lg shadow-sm border p-6">
                      <h3 className="text-lg font-semibold mb-4 text-foreground">Edit Stages</h3>
                      <Reorder.Group
                        axis="y"
                                    ...
                                    <AlertDialog>
                                      ...
                                      <AlertDialogContent>
                                        <AlertDialogHeader>
                                          ...
                                          <AlertDialogDescription>
                                            Are you sure you want to delete this stage? All
                                            information under this stage will be deleted forever!
                                          </AlertDialogDescription>
                                        </AlertDialogHeader>
                                        <AlertDialogFooter>
                                          <Button
                                          ...
                                          <Button
                                            variant="destructive"
                                            onClick={() => {
                                              const newStages = reorderedStages.filter(
                                                (_, i) => i !== index
                                              );
                                              setReorderedStages(newStages);
                                              setIsOpen(false);
                    ...
                    <div className="bg-card rounded-lg shadow-sm border p-6 space-y-4">
                      <h3 className="text-lg font-semibold mb-4 text-foreground">Team Stages</h3>
              ...
              <CardContent>
                {projectMembers && projectMembers.length > 0 ? (
                  <div className="flex flex-col gap-3">
                        ...
                        return (
                          ...
                          >
                            <Avatar className="h-10 w-10">
                              <AvatarImage
                                src={memberClerkAvatarUrl(member) || undefined}
                                alt=""
                    ...
                    {isAdmin && <AdminUserParticipation />}
                  </div>
                ) : (
      ...
      <AlertDialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        <AlertDialogContent className="w-full max-w-md">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Enter the details for the new stage.</AlertDialogDescription>
          </AlertDialogHeader>
          <div className="p-4">
            <Label htmlFor="stage-title" className="text-sm font-medium text-foreground">
              Stage Title
            </Label>
            <input
              id="stage-title"
