...
const STEPS = ["Welcome", "Notifications", "Soft Skills", "Target Industry", "Aspirations"];
...
const AppOnboarding = () => {
  ...
  const [userData, loading, error] = useDocument(userEmail ? doc(db, "users", userEmail) : null);
  ...
  const toggleSoftSkill = (tag: string) => {
    setSoftSkills((prev) => (prev.includes(tag) ? prev.filter((t) => t !== tag) : [...prev, tag]));
  };
  ...
  const filteredSoft = appTags
    .filter((t) => t.toLowerCase().indexOf(searchSoft.toLowerCase()) !== -1)
    .sort();
  const filteredIndustry = appTags
    .filter((t) => t.toLowerCase().indexOf(searchIndustry.toLowerCase()) !== -1)
    .sort();
    ...
    <AlertDialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <AlertDialogContent className="max-h-[90vh] w-full max-w-2xl overflow-y-auto">
        ...
        {securityCode ? (
          <>
            <AlertDialogHeader>
              <AlertDialogTitle>Save your recovery code</AlertDialogTitle>
              <AlertDialogDescription>
                Copy this code and store it somewhere safe. You may need it for account recovery. We
                will not show it again.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <div className="flex items-center gap-2 rounded-lg border bg-muted p-4">
              <code className="flex-1 text-xl font-mono tracking-wider">{securityCode}</code>
              <Button type="button" variant="outline" size="icon" onClick={copySecurityCode}>
          ...
          <>
            {page === 0 && (
              <>
                <AlertDialogHeader>
                  <AlertDialogTitle>Welcome to the onboarding survey</AlertDialogTitle>
                  <AlertDialogDescription>
                    Please take a minute to complete this form. It helps us with matching and
                    teammate recommendations.
                    <br />
                    <br />
                    <span className="block italic">
                      *This information will be kept private and will not be shared.
                    </span>
                  </AlertDialogDescription>
                </AlertDialogHeader>
              </>
            ...
            {page === 1 && (
              <>
                <AlertDialogHeader>
                  <AlertDialogTitle>Preferred notification methods</AlertDialogTitle>
                  <AlertDialogDescription>
                    Choose how you want to receive notifications. You can allow browser
                    notifications below.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <div className="space-y-3">
                  ...
                  <div>
                    <div className="flex items-center justify-between">
                      ...
                      <Button type="button" variant="ghost" size="sm" onClick={addBackupPhone}>
                    ...
                    {backupPhones.map((p, i) => (
                      <div key={i} className="mt-2 flex gap-2">
                        <Input
                          ...
                          value={p}
                          onChange={(e) => setBackupPhoneAt(i, e.target.value)}
                        />
                        <Button
                          type="button"
                          variant="ghost"
                          size="icon"
                          onClick={() => removeBackupPhone(i)}
                        >
                          <X className="h-4 w-4" />
                        </Button>
                      </div>
                  ...
                  <div className="border-t pt-4">
                    <div className="flex flex-wrap items-center gap-3">
                      {(["email", "phone", "both"] as NotificationPreference[]).map((opt) => (
                        <Button
                          key={opt}
                          type="button"
                          variant={notificationPreference === opt ? "default" : "outline"}
                          onClick={() => setNotificationPreference(opt)}
                        >
                          {opt === "email" ? "Email only" : opt === "phone" ? "Phone only" : "Both"}
                        </Button>
                      ))}
            ...
            {page === 2 && (
              <>
                <AlertDialogHeader>
                  <AlertDialogTitle>Soft skills</AlertDialogTitle>
                  <AlertDialogDescription>
                    Select the soft skills that describe you (at least one).
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <Input
                ...
                <div className="flex max-h-64 flex-wrap gap-2 overflow-y-auto">
                  {filteredSoft.map((tag) => (
                    <Button
                      key={tag}
                      type="button"
                      variant={softSkills.includes(tag) ? "default" : "outline"}
                      size="sm"
                      onClick={() => toggleSoftSkill(tag)}
                    >
                      {tag}
                    </Button>
                  ))}
            ...
            {page === 3 && (
              <>
                <AlertDialogHeader>
                  <AlertDialogTitle>What industry do you want to get into?</AlertDialogTitle>
                  <AlertDialogDescription>
                ...
                <div className="flex max-h-64 flex-wrap gap-2 overflow-y-auto">
                  {filteredIndustry.map((tag) => (
                    <Button
                      key={tag}
                      type="button"
                      variant={targetIndustry.includes(tag) ? "default" : "outline"}
                      size="sm"
            ...
            {page === 4 && (
              <>
                <AlertDialogHeader>
                  <AlertDialogTitle>Aspirations or dreams</AlertDialogTitle>
