  ...
  const handleGenerateTasks = async () => {
    ...
    if (!teamCharterResponses || teamCharterResponses.length < 5) {
      const errorInfo: ErrorInfo = {
        ...
        details:
          "Team charter must be completed before generating tasks. Please fill out all required questions in the Team Charter section.",
        timestamp: new Date(),
        canRetry: false,
    ...
    try {
      ...
      if (!allMembers || allMembers.length === 0) {
        const errorInfo: ErrorInfo = {
          ...
          details:
            "Please add team members to the project before generating tasks. You can invite members through the project settings.",
          timestamp: new Date(),
          canRetry: false,
          onDismiss: () => setTaskGenerationError(null),
        };
      ...
      >
        ...
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            {generatedOutput ? (
              <DialogDescription>Confirm the tasks structure generated.</DialogDescription>
            ) : (
              <DialogDescription>
          ...
          {taskGenerationError && <ErrorDisplay error={taskGenerationError} className="mb-4" />}
                          ...
                          <AccordionContent>
                            <div className="space-y-3">
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <p className="text-sm">
                                  <strong className="text-blue-600">Soft Deadline:</strong>{" "}
                                  <span className="font-medium">{task.soft_deadline}</span>
                                </p>
                                <p className="text-sm">
                                  <strong className="text-red-600">Hard Deadline:</strong>{" "}
