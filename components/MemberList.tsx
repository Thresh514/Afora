...
const MemberList = ({
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        const result = await batchInQueryForHooks(collection(db, "users"), "__name__", allUsers);
        setResults(result);
      } catch (err) {
  ...
  const teams = useMemo(() => {
    ...
    return projects.map((proj): ProjectTeam => {
      ...
      return {
        ...
        admins: Array.isArray(projectData.admins) ? projectData.admins : [],
        teamSize: typeof projectData.teamSize === "number" ? projectData.teamSize : 3,
      };
    });
  }, [projects]);
  ...
  const filteredAdmins = useMemo(() => {
    ...
    return admins.filter((admin) => admin.toLowerCase().includes(searchTerm.toLowerCase()));
  }, [admins, searchTerm]);
  ...
  const handleMemberMove = useCallback(
    async (memberEmail: string, fromProjectId: string | null, toProjectId: string | null) => {
      try {
        ...
        if (toProjectId) {
          ...
          const result = await addProjectMember(toProjectId, memberEmail, "member");
          if (!result.success) {
  ...
  const renderMemberCard = useCallback(
    (memberEmail: string, isAdmin: boolean, showActions: boolean = false, projectId?: string) => {
      const pfpData = isAdmin ? adminsPfp : membersPfp;
      const clerkUrl = clerkAvatarByEmail[normalizeMemberEmail(memberEmail)] || "";
      const imageSrc =
        clerkUrl ||
        pfpData[memberEmail] ||
                  ...
                  <SelectContent>
                    {projectTeams.map((team: ProjectTeam) => {
                      ...
                      const isAlreadyMember =
                        team.members.includes(memberEmail) ||
  ...
  const assignedMembers = projectTeams.reduce(
    (total: number, team: ProjectTeam) =>
      total + (team.members?.length || 0) + (team.admins?.length || 0),
    0
  );
    ...
    <div className="flex h-auto overflow-hidden rounded-lg bg-gradient-to-br from-background to-muted/40">
      ...
      <div className="w-80 bg-card border-r border-border flex flex-col shadow-lg">
              ...
              <>
                <div className="bg-white bg-opacity-20 backdrop-blur-sm p-3 rounded-lg">
                  <div className="text-2xl font-bold">
                    {projectTeams.reduce(
                      (total: number, team: ProjectTeam) => total + team.teamSize,
                      0
                    )}
                  </div>
                  <div className="text-xs opacity-90">Total Spots</div>
                </div>
                <div className="bg-white bg-opacity-20 backdrop-blur-sm p-3 rounded-lg">
                  <div className="text-2xl font-bold">{assignedMembers}</div>
                  <div className="text-xs opacity-90">Assigned</div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <div className="space-y-4">
            {/* Overview Card */}
            <div className="text-sm font-medium text-muted-foreground">Group Overview</div>
            <Card className="border-0 shadow-sm">
              <CardContent className="p-4">
                <div className="space-y-3">
                  ...
                  <Separator />
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">Available Members</span>
            ...
            <div className="space-y-3">
              <div className="text-sm font-medium text-muted-foreground">
                {userRole === "admin" ? "Team List" : "My Teams"}
              </div>
              ...
              {userRole === "admin" && (
                ...
                >
                  <div className="flex items-center gap-3">
                    <Users
                      className={`h-5 w-5 ${
                        selectedProject === "all" ? "text-green-600" : "text-green-400"
                      }`}
                    />
                    <div className="flex-1">
                      <div
                        className={`font-medium ${
                          selectedProject === "all" ? "text-green-900" : "text-green-700"
                        }`}
                      >
                        All Team Members
                      </div>
                      <div className="text-sm text-green-600">{totalMembers} members</div>
                    </div>
                    {selectedProject === "all" && <ArrowRight className="h-4 w-4 text-green-600" />}
                  </div>
                </div>
              )}
              {userAssignedProjects.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <h3 className="text-sm font-medium text-foreground mb-2">
                    {userRole === "admin" ? "No teams" : "You have not been assigned to any teams"}
                  </h3>
                ...
                userAssignedProjects.map((team: ProjectTeam) => (
                  <div
                    key={team.projectId}
                    className={`p-4 rounded-xl cursor-pointer transition-all duration-200 ${
                      ...
                      <div className="flex-1 min-w-0">
                        <div
                          className={`font-medium truncate ${
                            selectedProject === team.projectId ? "text-blue-900" : "text-foreground"
                          }`}
                        >
                          {team.projectTitle}
                        </div>
      ...
      <div className="flex flex-1 flex-col bg-card">
        ...
        <div className="bg-card p-6 border-b border-border">
          {selectedProject === "all" ? (
            <div>
              <h3 className="text-lg font-semibold text-foreground">All Team Members</h3>
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {(selectedProjectData?.members.length || 0) +
                    (selectedProjectData?.admins?.length || 0)}{" "}
                  team members
                </p>
              </div>
            ...
            <div>
              <h3 className="text-lg font-semibold text-foreground">
                {userRole === "admin" ? "Select Team" : "My Team Members"}
              </h3>
              <p className="text-sm text-muted-foreground">
                {userRole === "admin"
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {selectedProject === "all" ? (
                ...
                <div className="grid gap-3">
                  {filteredAdmins.length > 0 ? (
                    filteredAdmins.map((admin: string) => (
                      <div key={`admin-${admin}`}>{renderMemberCard(admin, true, true)}</div>
                    ))
                  ) : searchTerm ? (
            ...
            <div className="space-y-6">
              ...
              {selectedProjectData && (
                <div>
                  <h4 className="text-sm font-medium text-foreground mb-4">Current Team Members</h4>
                  <div className="grid gap-3">
                    {/* 显示项目管理员 */}
                    {selectedProjectData.admins?.map((admin: string) => (
                      <div key={`admin-${admin}`}>
