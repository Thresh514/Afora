...
const OrgHeader = ({ id }: OrgHeaderProps) => {
  ...
  const [data] = useDocument(userEmail ? doc(db, "users", userEmail, "orgs", id) : null);
          ...
          <div className="flex justify-between items-end">
            ...
            <div className="flex">
              <div className="backdrop-blur-md bg-white/75 rounded-xl p-4 inline-block">
                <h1
                  title={orgData.title}
                  className="text-5xl font-lighter tracking-wide text-foreground mb-2 truncate"
                >
                  {orgData.title.slice(0, 60).trim() + (orgData.title.length > 60 ? "..." : "")}
                </h1>
                {orgData.description && (
                  <p
                    title={orgData.description}
                    className="text-foreground text-lg max-w-2xl tracking-wide pl-2 truncate"
                  >
                    {orgData.description}
                  </p>
                )}
