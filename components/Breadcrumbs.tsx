...
function Breadcrumbs() {
  ...
  const stageDocRef = isStageRoute ? doc(db, "projects", segments[3], "stages", segments[5]) : null;
  const [stageDoc] = useDocument(stageDocRef);

  const taskDocRef = isTaskRoute
    ? doc(db, "projects", segments[3], "stages", segments[5], "tasks", segments[7])
    : null;
