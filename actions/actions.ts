...
export async function createNewUser(userEmail: string, username: string, userImage: string) {
  ...
  try {
    ...
    userRef.collection("notifications").add({
      ...
      message:
        "This is where you can view any updates about tasks or projects you're subscribed to.",
    });
  } catch (e) {
...
export async function createNewOrganization(orgName: string, orgDescription: string) {
  ...
  try {
    ...
    await adminDb.collection("users").doc(userEmail).collection("orgs").doc(docRef.id).set({
...
export async function deleteOrg(orgId: string) {
  ...
  try {
    ...
    const query = await adminDb.collectionGroup("orgs").where("orgId", "==", orgId).get();
...
export async function inviteUserToOrg(orgId: string, email: string, access: string) {
  ...
  try {
    ...
    await adminDb.collection("users").doc(email).collection("orgs").doc(orgId).set({
      userId: email,
      role: access,
      createdAt: Timestamp.now(),
      orgId,
...
async function getCurrentUserEmail(): Promise<string | undefined> {
  ...
  if (sessionClaims?.primaryEmailAddress && typeof sessionClaims.primaryEmailAddress === "string") {
...
export async function setUserOnboardingSurvey(payload: OnboardingPayload): Promise<{
  ...
  try {
    ...
    const securityCodePlain = crypto.randomBytes(5).toString("hex").toUpperCase();
    const securityCodeHash = crypto.createHash("sha256").update(securityCodePlain).digest("hex");
...
export async function updateProjects(orgId: string, groups: string[][]) {
  ...
  try {
    groups.map(async (group, index) => {
      ...
      await adminDb.collection("organizations").doc(orgId).collection("projs").doc(projectId).set({
        projId: projectId,
        members: group,
      });
  ...
  try {
    ...
    for (const adminEmail of finalAdmins) {
      try {
        ...
        console.error(`Failed to add project reference for admin ${adminEmail}:`, error);
      }
    }
...
export async function getProjectMembersResponses(projId: string) {
  ...
  try {
    ...
    const allMembers = [...(projectData?.members || []), ...(projectData?.adminsAsUsers || [])];
...
/** Re-opens a completed task: DB + stage stats + score/stats match what `completeTaskWithProgress` added. */
export async function restartTaskProgress(projId: string, stageId: string, taskId: string) {
  const { userId, sessionClaims } = await auth();
  ...
  try {
    ...
    if (!taskData?.isCompleted) {
      return {
        success: false,
        message: "Task is not completed; nothing to restart",
      };
    }
    ...
    const stageDoc = await stageRef.get();
    const stageData = stageDoc.data();
    const newStageCompleted = Math.max(0, (stageData?.tasksCompleted ?? 0) - 1);
    ...
    batch.update(stageRef, { tasksCompleted: newStageCompleted });

    if (!scoresQuery.empty) {
      const scoreRef = scoresQuery.docs[0].ref;
      const sd = scoresQuery.docs[0].data() as {
        total_points?: number;
        tasks_completed?: number;
        streak?: number;
      };
      // Mirrors double increment from updateUserScore(..., true) + updateUserTaskStats("completed")
      batch.set(
        scoreRef,
        {
          total_points: Math.max(0, (sd.total_points ?? 0) - points),
          tasks_completed: Math.max(0, (sd.tasks_completed ?? 0) - 2),
          streak: Math.max(0, (sd.streak ?? 0) - 1),
    ...
    return { success: true, message: "Task restarted at 0%" };
  } catch (error) {
...
export async function getTaskSubmissions(projId: string, stageId: string, taskId: string) {
  const { userId } = await auth();
  if (!userId) {
    throw new Error("Unauthorized");
  }
...
export async function getProjectStats(projId: string) {
  ...
  try {
    const stages = await adminDb.collection("projects").doc(projId).collection("stages").get();
...
export async function fixProjectAdmin(projId: string, userEmail: string) {
  try {
    const { userId } = await auth();
    if (!userId) {
      console.error("Auth failed: No userId");
...
function findOptimalAllocationWithBacktracking(projects: any[], availableMembers: number) {
  ...
  for (let i = 0; i < allocationScenarios.length; i++) {
    ...
    if (
      evaluation.filledProjects > maxFilledProjects ||
      (evaluation.filledProjects === maxFilledProjects &&
        evaluation.totalEmptySpots < minTotalEmptySpots)
    ) {
...
export async function previewSmartAssignment(orgId: string, teamSize?: number) {
  ...
  try {
    ...
    for (const member of unassignedMembers) {
      try {
        const memberDoc = await adminDb.collection("users").doc(member).get();
        if (memberDoc.exists) {
    ...
    const totalSpotsNeeded = projectsNeedingMembers.reduce(
      (sum, project) => sum + project.spotsAvailable,
      0
    );
    ...
    if (availableMembers >= totalSpotsNeeded) {
      ...
      optimalAllocation = projectsNeedingMembers.sort(
        (a, b) => b.spotsAvailable - a.spotsAvailable
      );
    } else {
      // Not enough members, use backtracking algorithm for optimal allocation
      optimalAllocation = findOptimalAllocationWithBacktracking(
        projectsNeedingMembers,
        availableMembers
      );
    }
        ...
        try {
                  ...
                  try {
                    ...
                    if (orgDoc.exists) {
                      projOnboardingSurveyResponse =
                        orgDoc.data()?.projOnboardingSurveyResponse || [];
                    }
                  } catch (error) {
                    console.error(
                      `Error fetching project survey for existing member ${memberEmail}:`,
                      error
                    );
                  }
          ...
          try {
            ...
            const aiResult = shouldConsiderExistingTeam
              ...
              : await matching(actualAllocationSize, allQuestions, newMembersInput);
            ...
            if (parsedResult.groups && parsedResult.groups.length > 0) {
              ...
              assignedEmails = selectedGroup
                .map((emailOrName: string) => {
                  // Find the matching member by email or name
                  const member = remainingMembers.find(
                    (m) =>
                      m.email === emailOrName ||
                      m.name === emailOrName ||
                ...
                .filter(
                  (email: string) => email && remainingMembers.some((m) => m.email === email)
                );
              ...
              if (parsedResult.compatibility_score !== undefined) {
                ...
                parsedResult.groups &&
                parsedResult.groups.length > 0 &&
                parsedResult.groups[0].score !== undefined
              ) {
              ...
              console.warn(
                `AI matching returned no groups for project ${project.id}, falling back to random`
              );
    ...
    const projectsNeedingMembers = projects
      .map((project) => {
        ...
        const allCurrentMembers = [
          ...new Set([...currentMembers, ...currentAdmins, ...participatingAdmins]),
        ];
...
export async function getTeamCompatibilityScores(orgId: string, projectId?: string) {
  ...
  try {
    let query = adminDb.collection("team_compatibility_scores").where("org_id", "==", orgId);
  ...
  try {
    ...
    for (const stageDoc of stagesSnapshot.docs) {
      ...
      stageStats.push({
        ...
        completion_rate:
          stageTasks.length > 0 ? (stageCompletedTasks / stageTasks.length) * 100 : 0,
      });
    }
...
export async function unassignTask(projId: string, stageId: string, taskId: string) {
  const { userId } = await auth();
  if (!userId) {
    throw new Error("Unauthorized");
  }
...
export async function reassignTask(
  ...
  try {
    ...
    const userDoc = await adminDb.collection("users").doc(newAssigneeEmail).get();
    if (!userDoc.exists) {
      throw new Error("User not found");
    }
...
export async function saveTeamAnalysis(projId: string, analysis: any) {
  const { userId } = await auth();
  if (!userId) {
    throw new Error("Unauthorized");
  }
...
export async function updateUserMatchingPreference(allowMatching: boolean) {
  const { userId } = await auth();
  if (!userId) {
    throw new Error("Unauthorized");
  }
...
export async function toggleAdminUserParticipation(projId: string, participate: boolean = true) {
  try {
    const { userId, sessionClaims } = await auth();
    if (!userId) {
      console.error("Auth failed: No userId");
    ...
    if (participate) {
      ...
      // 从参与用户活动的admin列表中移除
      updatedAdminsAsUsers = updatedAdminsAsUsers.filter((email: string) => email !== userEmail);
    }
  ...
  try {
    ...
    for (const update of updates) {
      ...
      if (!projectOrgId || projectOrgId !== orgId) {
        throw new Error(`Project ${projectId} does not belong to organization ${orgId}`);
      }
      ...
      const currentParticipants = new Set([...currentMembers, ...currentAdmins]);
      ...
      for (const email of newParticipants) {
        if (!currentParticipants.has(email)) {
          try {
            ...
            console.warn(`Failed to add project reference for user ${email}:`, error);
          }
        }
      }
      ...
      for (const email of currentParticipants) {
        if (!newParticipants.has(email)) {
          try {
            ...
            console.warn(`Failed to remove project reference for user ${email}:`, error);
          }
        }
      }
    }
...
function serializeTaskForClient(
  task: UserTaskWithContext & Record<string, unknown>
): UserTaskWithContext {
...
export async function getUserIncompleteTasks(searchQuery?: string): Promise<{
  ...
  try {
    ...
    const orgIds = userOrgsSnapshot.docs.map((doc) => doc.data().orgId).filter(Boolean) as string[];
